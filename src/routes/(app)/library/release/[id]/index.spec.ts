...
describe('get Last.FM API specific album info', () => {
  // Last.FM API data are very changeable, so a simple fix (or mistake) made by users on e.g. track duration may cause this test to fail if outdated
  it('compares result of album "Pink Floyd - Wish You Were Here" to given json', async () => {
    expect(JSON.stringify(await _fetchData('Pink Floyd;Wish You Were Here'))).toBe(
      '{"id":"Pink%20Floyd;Wish%20You%20Were%20Here","name":"Wish You Were Here","imgUrl":"https://lastfm.freetls.fastly.net/i/u/300x300/394cfbc6b2a74766a4364778c641ca51.png","artist":"Pink Floyd","tags":["progressive rock","1975","classic rock","70s","rock"],"tracks":[{"name":"Shine on You Crazy Diamond","duration":1052,"number":1},{"name":"Welcome to the Machine","duration":446,"number":2},{"name":"Have a Cigar","duration":308,"number":3},{"name":"Wish You Were Here","duration":340,"number":4},{"name":"Shine On You Crazy Diamond (Part Two)","duration":742,"number":5}]}'
    )
  }, 10000)

  it('compares result of album "The Notorious B.I.G. - Ready to Die The Remaster" to given json', async () => {
    expect(JSON.stringify(await _fetchData('The Notorious B.I.G.;Ready to Die The Remaster'))).toBe(
