			...
			{#each randPkData as piko}
				<div class="rounded overflow-hidden shadow-lg item p-2">
					...
					<div class="px-6 pt-4 pb-2">
						<a draggable="false" href="/pikodex/1?type={piko?.types[0]}" on:auxclick={()=>gotoNewTab(`/pikodex/1?type=${piko?.types[0]}`)}>
						...
						<a draggable="false" href="/pikodex/1?type={piko?.types[1]}" on:auxclick={()=>gotoNewTab(`/pikodex/1?type=${piko?.types[1]}`)}>
